...
namespace OldManInTheShopServer.Data.MySql
{
    ...
    public class MySqlDataManipulator : IDisposable
    {
        ...
        public bool DropSchema(string schemaName)
        {
            var cmd = Connection.CreateCommand();
        ...
        public UserToTextEntry GetForumPostById(int companyId, int repairJobId, int forumPostId)
        {
            string tableName = TableNameStorage.CompanyForumTable.Replace("(n)", companyId.ToString());
            tableName = tableName.Replace("(m)", repairJobId.ToString());
        ...
        {
            return OverallUser.Manipulator.RemoveDataWhere(Connection, TableNameStorage.OverallUserTable, "Email=\"" + email + "\"") != 0;
        }
        ...
        public bool UpdateUsersLoginToken(OverallUser toUpdate, LoginStatusTokens update)
        {
            ...
            var cmd = Connection.CreateCommand();
            cmd.CommandText = "update " + TableNameStorage.OverallUserTable + " set LoginStatusTokens=\"" + tokens + "\" where Email = \"" + toUpdate.Email + "\";";
            int res;
            try
            {
                res = cmd.ExecuteNonQuery();
